...
function initToolbar() {
    ...
    toolbar.on('click', function (e) {
        ...
        if (clickedElement.children() && clickedElement.children().hasClass('predefined__color')) {
            clickedElement = clickedElement.children();
        }
    ...
    locationButton.on('click', function (e) {
        ...
        awwCanvasWrapper.css('cursor', enableLocation ?
            'url(\'https://awwapp.com/static/site/app-assets/ic-location-enabled-cursor.png\') 11 11, auto' : 'crosshair');

        aww.setSendUserPointer(enableLocation);
        boardMenus.find('#aww-toolbar-pointer')[enableLocation ? 'addClass' : 'removeClass']('is--disabled');
        locationButton.find('.location--disabled')[enableLocation ? 'addClass' : 'removeClass'](HIDDEN_CLASS);
        locationButton.find('.location--enabled')[enableLocation ? 'removeClass' : 'addClass'](HIDDEN_CLASS);
...
function onExportMenuClick(e) {
    ...
    if (buttonId.indexOf('export-as-pdf') !== -1) {
        aww.getPDF((pdfLink, err) => { console.log('..', pdfLink, err); });
    } else if (buttonId.indexOf('export-as-image') !== -1) {
        aww.getMaxViewportImage((err, imageData) => { console.log('..', imageData); });
    }
}
