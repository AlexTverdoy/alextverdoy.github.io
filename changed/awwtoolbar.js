...
// Download color wheel image file for custom color picker
// https://awwapp.com/static/site/app-assets/color-wheel.png <-- color wheel image sample
const COLOR_WHEEL_PATH =
  "https://awwapp.com/static/site/app-assets/color-wheel.png";
...
function initToolbar() {
  ...
  aww.redoAvailable(showRedoButton);
  aww.onZoomLevelUpdate(updateZoomLevel);
  ...
  awwContainer = document.querySelector(".aww");
  awwCanvasWrapper = $(".aww-canvas-wrapper");
  ...
  redoButton = boardMenus.find("#aww-toolbar-redo");
  ...
  toolbar.on("click", function (e) {
    ...
    if (clickedElement.parent().hasClass("tools__item--button")) {
      ...
      aww.getCurrentUser(user => {
        ...
        let toolId = (parentElement.attr("id") || "").substring(
          toolbarPrefix.length
        );
        ...
        if (
          ...
          mainToolbarButtonClicked(
            toolsInToolbar[toolId].currentTool,
            parentElement
          );
        } else {
      ...
      let urlField = clickedElement
        .parent()
        .parent()
        .find("#aww-toolbar-url-for-upload");
      let originalSrc = urlField.val();
      let fileUrl =
        "https://converter.awwapp.com/api/v2/get_picture/?url=" +
        encodeURIComponent(originalSrc);

      if (fileUrl.match(/\.(jpe?g|png)$/gi)) {
        aww.uploadImageFromUrl(fileUrl, function () {});
      } else if (fileUrl.match(/\.(ppt|pdf)$/gi)) {
  ...
  image.src = COLOR_WHEEL_PATH;

  var exportMenuDropdown = boardMenus.find("#export-menu-button");
  var exportMenu = boardMenus.find("#export-menu");
  addDropdownClickEvent(exportMenuDropdown, exportMenu, true);

  exportMenu.on("click", onExportMenuClick);
  // minimap
  var minimap = boardMenus.find(".tools__minimap--map");
  var minimapToggle = boardMenus.find(".tools__minimap--button");
  var minimapVisibility = minimapToggle.find(".tools__minimap--visibility");
  var zoomIn = boardMenus.find("#zoom-in");
  var zoomOut = boardMenus.find("#zoom-out");
  var centerButton = boardMenus.find("#center");
  var locationButton = boardMenus.find("#location");

  minimapToggle.on("click", function (e) {
    e.preventDefault();
    if (minimapVisibility.attr("class").indexOf(OPEN_CLASS) !== -1) {
      minimapVisibility.removeClass(OPEN_CLASS);
      minimap.removeClass(VISIBLE_CLASS);
      aww.minimapToggle(false);
    } else {
      minimapVisibility.addClass(OPEN_CLASS);
      minimap.addClass(VISIBLE_CLASS);
      aww.minimapToggle(true);
    }
  });
  ...
  panButton.on("click", function (e) {
    e.preventDefault();
    if (panButton.attr("class").indexOf(SELECTED_CLASS) === -1) {
      panButton.addClass(SELECTED_CLASS);
    }
    aww.setTool("pan");
  });
  ...
  centerButton.on("click", function (e) {
    e.preventDefault();
    aww.fitContentToScreen();
  });

  locationButton.on("click", function (e) {
    e.preventDefault();

    var enableLocation = !locationButton.attr("class").includes(SELECTED_CLASS);

    awwCanvasWrapper.css(
      "cursor",
      enableLocation
        ? "url('https://awwapp.com/static/site/app-assets/ic-location-enabled-cursor.png') 11 11, auto"
        : "crosshair"
    );

    aww.setSendUserPointer(enableLocation);
    boardMenus
      .find("#aww-toolbar-pointer")
      [enableLocation ? "addClass" : "removeClass"]("is--disabled");
    locationButton
      .find(".location--disabled")
      [enableLocation ? "addClass" : "removeClass"](HIDDEN_CLASS);
    locationButton
      .find(".location--enabled")
      [enableLocation ? "removeClass" : "addClass"](HIDDEN_CLASS);
    locationButton[enableLocation ? "addClass" : "removeClass"](SELECTED_CLASS);
  });
...
function goFullscreen() {
  if (
    ...
    if (awwContainer.requestFullscreen) {
      awwContainer.requestFullscreen();
    } else if (awwContainer.webkitRequestFullscreen) {
      awwContainer.webkitRequestFullscreen();
    } else if (awwContainer.mozRequestFullScreen) {
      awwContainer.mozRequestFullScreen();
    } else if (awwContainer.msRequestFullscreen) {
      awwContainer.msRequestFullscreen();
    }

    fullscreenIcon.attr("src", ASSETS_PATH + "/ic-exit-fullscreen.svg");
    isInFullscreen = true;
  }
}
...
function exitFullscreen() {
  ...
  fullscreenIcon.attr("src", ASSETS_PATH + "/ic-fullscreen.svg");
  isInFullscreen = false;
}
...
function getExportPdfMessage(isAdmin) {
  return (
    "Please, don't close your browser until we export your PDF. " +
...
function onExportMenuClick(e) {
  ...
  if (buttonId.indexOf("export-") === -1) return;

  if (buttonId.indexOf("export-as-pdf") !== -1) {
    aww.getPDF((pdfLink, err) => {
      console.log("..", pdfLink, err);
    });
  } else if (buttonId.indexOf("export-as-image") !== -1) {
    aww.getMaxViewportImage((err, imageData) => {
...
function calculateChatTextareaHeight() {
  chatInput
    .parent()
    .css("height", Math.max(48, Math.min(100, chatInput.scrollHeight)) + "px");
}
...
function handleMessages(message, uid, time) {
  aww.getCurrentUser(me => {
    ...
    if (!lastTimestampTime || (lastTimestampTime - time) / 1000 > 2) {
      ...
      let timestamp = $(
        '<time class="message__timestamp">' + formattedTime + "</time>"
      );
...
function changeTool(data) {
  ...
  if (parentId && !parentId.includes("upload")) {
    if (currentlySelectedTool) {
      ...
      let prevSelectedToolParent = toolbar
        .find("#" + toolbarPrefix + currentlySelectedTool.key)
        .children()
        .first();
      let prevSelectedTool = toolbar
        .find("#" + toolbarPrefix + currentlySelectedTool.value.currentTool)
        .children()
        .first();
      if (prevSelectedTool) prevSelectedTool.removeClass(SELECTED_CLASS);
      if (prevSelectedToolParent)
        prevSelectedToolParent.removeClass(SELECTED_CLASS);
    }

    toolsInToolbar[parentId].currentTool = toolId;
    toolsInToolbar[parentId].selected = true;
    let toolParent = toolbar
      .find("#" + toolbarPrefix + parentId)
      .children()
      .first();
    let thumb = toolParent ? toolParent.find("img").first() : null;
    let selectedTool = boardMenus
      .find("#" + toolbarPrefix + toolId)
      .children()
      .first();
...
function mainToolbarButtonClicked(toolId) {
  ...
  if (toolId.includes("undo")) {
    ...
    aww.uploadPresentationFromComputer(
      presentationUrl =>
        console.log("Presentation is uploaded to server...", presentationUrl),
...
function penDownHandler(e) {
  if ($(e.target).hasClass("slide--dot")) {
    ...
    sliderParentId = (sliderParent.attr("id") || "").substring(
      toolbarPrefix.length
    );
    ...
    relativeStartingPoint = [
      movingSlider.offset().left - colorWheelCanvas.offset().left,
      movingSlider.offset().top - colorWheelCanvas.offset().top
    ];
    ...
    if (sliderParentId !== "colors") {
      ...
      sliderIndex = Math.max(
        0,
        SIZES.indexOf(toolsInToolbar[sliderParentId].size)
      );
    }
    ...
    let currentYPos =
      ...
      dotHalfSize;
    let canvasScale =
      colorWheelCanvas[0].getBoundingClientRect().width /
      colorWheelCanvas.width();

    let margin = limitInsideCircle(
      currentXPos - 2 * dotHalfSize,
      currentYPos - 2 * dotHalfSize,
      (colorWheelSize / 2) * canvasScale,
      (colorWheelSize / 2) * canvasScale,
      (colorWheelSize / 2) * canvasScale
    );
    colorWheelPicker.css("margin-left", margin.x + "px");
    colorWheelPicker.css("margin-top", margin.y + "px");
    let imageData = colorWheelCanvasCtx.getImageData(
      currentXPos / canvasScale - 2 * dotHalfSize,
      currentYPos / canvasScale - 2 * dotHalfSize,
      1,
      1
    );
...
function penMoveHandler(e) {
  if (movingSlider) {
    ...
    if (sliderParentId === "colors") {
      let margin = limitInsideCircle(
        relativeStartingPoint[0] + dx,
        relativeStartingPoint[1] + dy,
        colorWheelSize / 2,
        colorWheelSize / 2,
        colorWheelSize / 2
      );
      ...
      if (SIZES[sliderTick + sliderIndex]) {
        ...
        movingSlider.css(
          "margin-left",
          (sliderTick + sliderIndex) * sliderPercTick + "%"
        );
...
function addColorEl(color, colorIndex, isSelected) {
  ...
  newColorEl = $(
    '<div class="predefined__color" style="background-color: ' + color + '">'
  );
  let newEl = $(
    '<li class="tools__item" id="aww-toolbar-color-' + colorIndex + '"></li>'
  ).append(
    $(
  ...
  newEl.insertBefore(
    customColorsRow.children()[customColorsRow.children().length - 1]
  );
}
...
function reorderCustomColors() {
  customColors = customColors.slice(
    Math.max(0, customColors.length - MAX_CUSTOM_COLORS)
  );
...
function setupSlider(toolbox) {
  ...
  if (slider) {
    ...
    let sliderIndex = Math.max(
      0,
      SIZES.indexOf(
        toolsInToolbar[openedToolboxParentId.substring(toolbarPrefix.length)]
          .size
      )
    );
...
function showRedoButton(change) {
  redoButton[change ? "removeClass" : "addClass"](HIDDEN_CLASS);
}
